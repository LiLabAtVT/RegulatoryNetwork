...
setwd(workingDir); 
getwd();
fileList=list.files(workingDir, pattern="_ave.csv")
#-----------------------------
# 1. load dataset
...
#topVarGenes <- head(exp.data_rowvars[order(-exp.data_rowvars[,'rowvars']),], (0.25*dim(exp.data_rowvars)[1]) )
...
#-----------------------------
# transposing matrix for minet input
...
dim(clr.list_sorted)
head(clr.list_sorted)
clr.list_sorted_filtered=clr.list_sorted[(clr.list_sorted[,3] >0.1), ]
dim(clr.list_sorted_filtered)

#write.table(format(clr.list_sorted_filtered,digit=4), file = paste0(gsub(".csv","",InputFileName),"_minet_clr_list.csv"),row.names=F, quote = FALSE)
